function longestCommonSubstringAll(strings) {
  ...
  for (let len = base.length; len >= 1; len--) {
    for (let start = 0; start <= base.length - len; start++) {
      const substr = base.slice(start, start + len);
...
if (require.main === module) {
  ...
  rl.question('Enter a list of strings (space-separated): ', (input) => {
    ...
    const result = longestCommonSubstringAll(strings);
    console.log(`✅ Length of longest common substring in all strings: ${result}`);
    rl.close();
  });
}

module.exports = longestCommonSubstringAll;
